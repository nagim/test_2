...
the traffic case. When the `rps` is set to `0.0`, the registration does not
start until a greater value is set.
+
For example, when 10000 entities are defined in the traffic case and 100
entities register per second (`rps` := `100.0`), then it takes 100 seconds
...
time. The expiration time is sent to the IMS system in the [blue]#_Expires_#
header of the [blue]#REGISTER# message. Upon registering, the entity sends the
initial value set in the `registrationExpires` parameter to the IMS
system. The real expiration time is the value included in the [blue]#200 OK#
...
is to send [blue]#re-REGISTER# messages).
+
Unit: 1/sec
+
...
includes a [blue]#_Retry-After_# header with the suggested waiting time until the
next try. It sets the maximum number of consecutive responses received
including a [blue]#_Retry-After_# header before a call is marked as failed.
...
again the first time after an error, when there is a [blue]#_Retry-After_# field
...
+
Used to set the content of the [blue]#_P-Access-Network-Info_# header of [blue]#REGISTER#
messages. If the default value is used or the parameter is omitted, no
[blue]#_P-Access-Network-Info_# header is added to the messages at all.
+
Allowed values:
...
Default value: _``false``_
+
When set to _``true``_ or _``omitted``_, the [blue]#_Contact_# header of the [blue]#REGISTER# messages
contains the transport protocol (for example Contact:
<…transport=udp…>). When set to _``false``_, the transport protocol is not
included in the [blue]#_Contact_# header.
+
* `distributeRegister`
+
Type: boolean
...
Default value: _``false``_
+
When set to true, the [blue]#_Authorization_# header is included in the initial
and the [blue]#de-REGISTER# messages.

> ==== Note::Note
> When AKA authentication is set, the [blue]#_Authorization_# header is always included in the initial and in the [blue]#de-REGISTER# messages, independently of this parameter.
...
Contact header of the [blue]#de-REGISTER# message (instead of all of them using
the * syntax in the [blue]#_Contact header_#, which is the case if the parameter
...
+
Enables the inclusion of keep parameter to [blue]#_Via_# header, and defines the
frequency of [blue]#REGISTER# messages that contains keep.
+
** `enable`
+
Type: boolean
...
+
The `reRegType` parameter defines whether the Call-ID in the [blue]#re-REGISTER#
messages remains, or the application generates new ones for every
[blue]#re-REGISTER# message. Its value can also be _``INITIAL``_ (new Call-IDs is
generated).
...
+
There is an [blue]#_Authorization_# header in a [blue]#REGISTER# message, if it is the
reply to a [blue]#401 – Unauthorized response#, or if `authHeaderInInitialReg`
parameter is enabled. The `OAuth` field is a record in the [blue]#_Authorization_#
...
[blue]#_Authorization_# header of the [blue]#REGISTER# messages. The `oauth` parameter is
...
+
An example for this parameter in an outgoing [blue]#REGISTER# message can be the
following:
+
`oauth="200013600"`
+
...
Default value: _``false``_
+
When set to _``true``_, after a [blue]#401# response, all [blue]#REGISTER# messages contain a
[blue]#_UserAgent_# header with the value _``dont_challenge``_.
...
+
It indicates the time delay when sending [blue]#UPDATE# message after the call
